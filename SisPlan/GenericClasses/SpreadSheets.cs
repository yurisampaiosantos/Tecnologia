...
namespace GenericClasses
{
    public class SpreadSheets
    {
                ...
                case "LM$":
                    {
                        ...
                        for (int i = 0; i < columns.Length - 1; i++)
                        {
                            ...
                            {
                                H.ProcessHistory = "A estrutura da planilha LM deve obedecer a estrutura abaixo: " + Convert.ToChar(13) + Convert.ToChar(10);
                        ...
                        for (int i = 0; i < dtSpreadsheet.Rows.Count; i++)
                        {
                            ...
                            if (dim == "") 
                            {
                                H.ProcessHistory = "Linha " + (i + 1).ToString() + " ==>  Coluna DIMENSION - em branco";
                                GenericClasses.Log.SaveLog(log, H); 
                                bRet = false; 
                            }
                            if(dim.IndexOf("mm.") < 0)
                            {
                                H.ProcessHistory = "Linha " + (i + 1).ToString() + " ==>  Coluna DIMENSION - Falta mm.";
                                GenericClasses.Log.SaveLog(log, H);
                                bRet = false;
                                break;
                            }
                            if (dim.IndexOf("..") >= 0)
                            {
                                H.ProcessHistory = "Linha " + (i + 1).ToString() + " ==>  Coluna DIMENSION fora de padrão";
                                GenericClasses.Log.SaveLog(log, H);
                                bRet = false;
                                break;
                            }
                ...
                case "LP$":
                    {
                        ...
                        for (int i = 0; i < columns.Length - 1; i++)
                        {
                            if (dtSpreadsheet.Columns[i].ColumnName.ToUpper() != columns[i].ToUpper())
                            {
                                H.ProcessHistory = "A estrutura da planilha LP deve obedecer a estrutura abaixo: " + Convert.ToChar(13) + Convert.ToChar(10);
                                GenericClasses.Log.SaveLog(log, H);
                                H.ProcessHistory = header;
                                GenericClasses.Log.SaveLog(log, H);
                                bRet = false;
        ...
        //============================================
        public static bool ValidaCodigoProduto(StreamWriter log, DataTable dtProjRcpt, string colunaProduto, string disciplina, string fileName, ref bool restricaoMaterial)
        {
            bool bRet = true;
            string strSQL = "";
            string codProduto = "";
            ...
            {
                codProduto = dtProjRcpt.Rows[i][colunaProduto].ToString().Trim();
                ...
                if (codProduto != "")
                {
                    ...
                    //strSQL = @"SELECT REPR_TIPO_DICIONARIO, DIPC_CODIGO, DIPU_CODIGO FROM V_RP_ITEM WHERE DIPU_CODIGO = '" + codProduto + @"'
                    //           UNION
                    //           SELECT REPR_TIPO_DICIONARIO, DIPC_CODIGO, DIPU_CODIGO FROM V_RP_ITEM WHERE DIPC_CODIGO = '" + codProduto + "'";

                    strSQL = @"SELECT DIPC_CODIGO FROM EEP_CONVERSION.DICIONARIO_PRODUTO_COMPOSTO WHERE DIPC_CODIGO = '" + codProduto + @"'
                               UNION
                               SELECT DIPU_CODIGO FROM EEP_CONVERSION.DICIONARIO_PRODUTO_UNICO WHERE DIPU_CODIGO = '" + codProduto + "'";
                    try
                    {
                        //DataSet ds = new DataSet();
                        ...
                        //}
                        //dt = ds.Tables[0];
                        dt = BLL.ProjTubRcptTeeweeBLL.Select(strSQL);
                    }
                    catch (Exception ex) { throw new Exception(ex.Message); }
                    //============================================================
                    //============================================================
                    ...
                    if (dt.Rows.Count == 0)
                    {
                        ...
                        switch (disciplina)
                        {
                            case "Tubulacao":
                            {
                                ...
                                msg += " | SPOOL: " + spool + " DESCARTADO";
                                msg += " | Contractor_CC: " + dtProjRcpt.Rows[i]["Contractor_CC"].ToString().Trim();
                                msg += " | Short_Desc: " + dtProjRcpt.Rows[i]["Short_Desc"].ToString().Trim();
                                msg += " | Pipeline: " + dtProjRcpt.Rows[i]["Pipeline"].ToString().Trim();
                                msg += " | PartDiam1: " + dtProjRcpt.Rows[i]["PartDiam1"].ToString().Trim();
                                msg += " | PartDiam2: " + dtProjRcpt.Rows[i]["PartDiam2"].ToString().Trim();
                                //GenericClasses.Tubulacao.DiscardSpool(spool, fileName);
                                break;
                            }
                            case "Eletrica":
                            {
                                msg = "Linha " + (i + 2).ToString() + " ==>  Coluna CODIGO - Material inválido: " + codProduto;
                                msg += " | DESCRICAO: " + dtProjRcpt.Rows[i]["DESCRICAO"].ToString().Trim();
                                msg += " | DIMENSAO: " + dtProjRcpt.Rows[i]["DIMENSAO"].ToString().Trim();
                                msg += " | TAG: " + dtProjRcpt.Rows[i]["TAG"].ToString().Trim();
                                break;
                            }
                            default:
                                break;
                        }
                        H.ProcessHistory = msg;
                        H.FileName = fileName;
                        GenericClasses.Log.SaveLog(log, H);
                        bRet = false;
                    }
        ...
        //============================================
        public static bool IsValidJusSpreadsheet(DataTable dtSpreadsheet, string jsFileType)
        {
            bool bRet = true;
            string[] colunas = new string[4];
            if (jsFileType == "ES") { colunas[0] = "ID_JUNTA"; colunas[1] = "PLATAFORMA"; colunas[2] = "MODULO"; colunas[3] = "BLOCO"; }
            else { colunas[0] = "NUMERO"; colunas[1] = "PLATAFORMA"; colunas[2] = "MODULO"; colunas[3] = "BLOCO"; }
            
            for (int i = 0; i < colunas.Length; i++)
            {
                bRet = false;
                for (int d = 0; d < dtSpreadsheet.Columns.Count; d++)
                {
                    if (colunas[i].ToUpper() == dtSpreadsheet.Columns[d].ColumnName.ToUpper())
                    {
                        bRet = true;
                        break;
                    }
