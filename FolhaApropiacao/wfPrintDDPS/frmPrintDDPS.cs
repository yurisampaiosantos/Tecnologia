    ...
    public partial class frmPrintDDPS : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (lbTime.Items.Count != 0 && lbDates.Items.Count != 0)
            {
                ...
                myDialogResult = MessageBox.Show("Será gerado um total de " + (lbTime.Items.Count * lbDates.Items.Count).ToString() + " pág(s), deseja imprimir?", "Impressão", MessageBoxButtons.YesNo);
        ...
        private void cbArea_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    TabelaNeg tabelaNeg = new TabelaNeg();
                    cbEquipe.DataSource = tabelaNeg.ListTeamArea(Convert.ToDecimal(cbArea.SelectedValue), DateTime.Now.Date.ToString("dd/MM/yyyy"), Convert.ToDecimal(cbGrupo.SelectedValue));
