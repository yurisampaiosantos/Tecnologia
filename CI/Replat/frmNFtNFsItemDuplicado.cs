...
namespace Replat
{
    public partial class frmNFtNFsItemDuplicado : Form
    {
        string quebraLinha = Environment.NewLine;

        public frmNFtNFsItemDuplicado()
        {
            InitializeComponent();
        }
        ...
        {
            string SQL = @"SELECT
                                EMPR_NOME, NOFS_NUMERO, NOFS_SERIE, DIPR_COD, NFSI_QTD, NFSI_VLR_UNIT, TNFS_DESCRICAO
                            FROM 
                                V_NFT_NFS_ITEM_DUPLICADO";
