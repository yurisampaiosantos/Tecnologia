...
namespace Replat
{
    public partial class frmSaldo : Form
    {
        ...
        {
            return @"SELECT 
                        ID_IMPORTACAO, ID_REF, COD_LOCATION, DATA, DATA_GER_LEG, ID_CORPORATIVO, ORGANIZACAO, PART_NUMBER, PROCEDENCIA_INFO, QTDE, SEGMENTO1, SEGMENTO2, SEGMENTO3, SEGMENTO4, SEGMENTO5
                    FROM 
                        V_SALDO_REPLAT";
        }
        ...
        private void btExportar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    frm.progressBar1.Maximum = gvImportacao.Rows.Count + gvImportacao2.Rows.Count;
                    ...
                    WorkSheet.Cells[1, 1] = "Análise dos Dados de Saldo Diário";
                    WorkSheet.Cells[2, 1].Select();
                    WorkSheet.Name = "Saldo";
                    WorkSheet2.Name = "Movimentos";

                    if (gvImportacao.Columns[4].Name != "ALERTAS")
                    {
                        WorkSheet2.Select();

                        //Cabeçalho WorkSheet
                        for (int i = 1; i <= gvImportacao2.Columns.Count; i++)
                        {
                            WorkSheet2.Cells[3, i] = gvImportacao2.Columns[i - 1].Name;
                        }
                        ...
                        for (int o = 0; o < gvImportacao2.Rows.Count; o++)
                        {
                            ...
                            {
                                if (Convert.ToString(gvImportacao2[j, o].Value) != null) WorkSheet2.Cells[o + 4, j + 1] = gvImportacao2[j, o].Value.ToString();
                            }
                        }

                        App.Columns.AutoFit();

                        WorkSheet2.Cells[1, 1] = "Análise dos Movimentos Tipo 'QL'";
                        WorkSheet2.Cells[2, 1].Select();
                    }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (myDialogResult == DialogResult.Yes)
                    {
                        ...
                        RfNfEntradaBLL.ExecuteSQLInstruction("TRUNCATE TABLE RF_SALDOS");
                        ...
                        string strSQL = @"INSERT INTO RF_SALDOS
                                        (ID_IMPORTACAO, ID_REF, COD_LOCATION, DATA, DATA_GER_LEG, ID_CORPORATIVO, ORGANIZACAO, PART_NUMBER, PROCEDENCIA_INFO, QTDE, SEGMENTO1, SEGMENTO2, SEGMENTO3, SEGMENTO4, SEGMENTO5)" + quebraLinha;
                        strSQL += SQL();
                        ...
                        MessageBox.Show("Saldo Diário e Movimentações tipo QL Disponíveis no IN-OUT", "SUCESSO", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
