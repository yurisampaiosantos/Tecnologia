...
namespace wfPrintFolhaApropriacao
{
    public partial class frmGenerationPDF : Form
    {
        ...
        private void frmGenerationPDF_Load(object sender, EventArgs e)
        {
            ...
            dataTable = tabelaNeg.ListTeam(teamId);
            ...
            reportParams.Add(new ReportParameter("date", Convert.ToDateTime(date).ToString("ddMMyyyy")));
            reportParams.Add(new ReportParameter("barCode", tabelaNeg.Barcode(teamId, date)));
            reportParams.Add(new ReportParameter("encarregado", lider));
            ...
            teamCode = dataTable.Rows[0]["TEAM_CODE"].ToString();
            reportParams.Add(new ReportParameter("idTime", teamCode));

            grupo = dataTable.Rows[0]["GROUP_TEAM_DESCRIPTION"].ToString();
            reportParams.Add(new ReportParameter("grupo", grupo));

            reportParams.Add(new ReportParameter("descriptionTime", teamDescription));
            reportParams.Add(new ReportParameter("activeDirect", System.Environment.MachineName + " - " + System.Environment.UserName + " - " + DateTime.Now.ToString() + " - Versão 1.3 (TeamId-" + teamId.ToString() + ")"));

            
            shift = dataTable.Rows[0][2].ToString();            
            reportParams.Add(new ReportParameter("shift", shift));
        ...
        private void Print()
        {
         ...
         //   ps.PrinterName = printDialog1.PrinterSettings.PrinterName;
            ...
            ps.DefaultPageSettings.PrinterSettings.DefaultPageSettings.Landscape = true;
